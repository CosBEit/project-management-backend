...
from server.dependencies.auth import OAuth2PasswordBearerWithCookie, create_csrf_token, get_password_hash, get_user
from server.modals.users import AddUserInputDataModel, RegisterUserInputDataModel
from server.configs.db import users_collection
from jose import jwt, JWTError
from server.dependencies.send_emails import send_invitation_email
...
async def add_user(add_user_data: AddUserInputDataModel, current_user: str = Depends(oauth2_scheme)):
    ...
    try:
        ...
        frontend_url = f"{os.getenv('frontend_url')}/create-account"
        ...
        users = await users_collection.find({}, {"email": 1, "role": 1, "status": 1, "created_at": 1}).to_list(length=None)
        for user in users:
...
async def update_user(user_id: str, user_data: dict, current_user: str = Depends(oauth2_scheme)):
    ...
    try:
        ...
        if user_data["email"] != existing_user["email"]:
            email_exists = await users_collection.find_one({"email": user_data["email"], "_id": {"$ne": user_id}})
            if email_exists:
                raise HTTPException(
                    status_code=status.HTTP_409_CONFLICT,
                    detail="Email address already exists.",
...
async def register_user(register_data: RegisterUserInputDataModel):
    """Register a user with email and password after verifying the token.

    Args:
        register_data (RegisterUserInputDataModel): The user's registration data including email and password.

    Returns:
        JSONResponse: A response indicating the registration status.

    Raises:
        HTTPException: If the token is invalid or an error occurs during registration.
    """
    try:
        ...
        if not user or user["status"] != "pending":
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="User not found or already registered")

        # Update the user with the password and account status
        password_hash = get_password_hash(register_data.password)
        await users_collection.update_one(
            {"email": email},
            {"$set": {"password": password_hash, "status": "active"}}
        )
